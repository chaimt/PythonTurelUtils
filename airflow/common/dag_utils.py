...
from core.dynamic_imports import (
    Settings,
    get_env_from_path,
    get_file_name,
    get_project_from_path,
    import_relative_source,
    is_test_env,
    is_test_mode,
)
...
def get_dag_id(file_name, version=None, prefix=None):
    ...
    return "{0}.{1}{2}.{3}_{4}".format(env, get_project_from_path(file_name), prefix, get_file_name(file_name), version)
...
class turelDAG(DAG):
    ...
    def __init__(self, dag_config, filename, dag_id, *args, **kwargs):
        ...
        self.log_callback(
            {"action": "create_dag", "dag_id": dag_id, "start_date": str(self.start_date), "schedule_interval": kwargs["schedule_interval"]},
            "create_dag",
        )
    ...
    def add_dags_args(dict_info, dag_config):
        ...
        dict_info["bq_conn_id"] = l_dic_utils.get_value_by_path(dag_config, "global.connections.datalake_bigquery_conn_id")

        dict_info["gcp_conn_id"] = l_dic_utils.get_value_by_path(dag_config, "global.connections.datalake_bigquery_conn_id")

        dict_info["airflow_project_id"] = l_dic_utils.get_value_by_path(dag_config, "airflow.project_id")

        dict_info["airflow_dataset_id"] = l_dic_utils.get_value_by_path(dag_config, "airflow.dataset_id")

        dict_info["airflow_bq_conn_id"] = l_dic_utils.get_value_by_path(dag_config, "airflow.bq_conn_id")
