...
def get_resource_file(file_name, resource_name):
    ...
    if di.is_test_mode():
        ...
        file_location = "dags/{env}/{project_file}/resources/{resource_file}".format(env=env, project_file=project_file, resource_file=resource_name)

        l_audit.turel_log("action=get_resource_file, bucket={0}, file_location={1}".format(di.Settings.get_instance().g_bq_bucket, file_location))
...
def get_config_file(file_name, resource_name, project_name=None):
    l_audit.turel_log("action=get_config_file, file_name={0}, resource_name={1}, project_name={2}".format(file_name, resource_name, project_name))
    ...
    try:
        ...
        try:
            ...
            l_audit.turel_log_exception(
                "action=application_base, file={}, service={}, section={}, error={}".format(file_name, service, section, str(e))
            )

        # load project-specific config:
        if service is not None:
            try:
                application_env = get_config_file(file_name, "application-{}.yml".format(service), di.get_project_from_path(file_name))

                if application_env is not None:
                    l_dic_utils.dict_merge(application_base, application_env)
            except Exception as e:
                l_audit.turel_log_exception(
                    "action=application_env, file={}, service={}, section={}, error={}".format(file_name, service, section, str(e))
                )
            ...
            try:
                ...
                l_audit.turel_log_exception(
                    "action=application_env_a, file={}, service={}, section={}, error={}".format(file_name, service, section, str(e))
                )

            # load test file
            if di.is_test_mode():
                try:
                    application_env = get_config_file(file_name, "application-{}-test.yml".format(service), di.get_project_from_path(file_name))
        ...
        try:
            ...
            l_audit.turel_log_exception(
                "action=application_version, file={}, service={}, section={}, error={}".format(file_name, service, section, str(e))
            )
        ...
        l_audit.turel_log_error("action=global, file={}, service={}, section={}, error={}".format(file_name, service, section, str(exc)))
    except Exception as e:
        l_audit.turel_log_exception("action=global, file={}, service={}, section={}, error={}".format(file_name, service, section, str(e)))
